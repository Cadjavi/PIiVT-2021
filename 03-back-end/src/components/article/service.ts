...
class ArticleService extends BaseService<ArticleModel> {
  ...
  private async getAllFeatureValuesByArticleId(
    articleId: number
  ): Promise<ArticleFeatureValue[]> {
    const sql = `
    SELECT
  ...
  private async getAllPricesByArticleId(
    articleId: number
    ...
    return new Promise<IErrorResponse>(async (resolve) => {
      ...
      this.db
        ...
        .then(async () => {
          ...
          throw {
            errno: -102,
            sqlMessage: "Could not delete article prices.",
          };
        })
        .then(async () => {
          ...
          throw {
            errno: -103,
            sqlMessage: "Could not delete article feature values.",
          };
        })
        .then(async () => {
          ...
          throw {
            errno: -104,
            sqlMessage: "Could not delete article cart records.",
          };
        })
        .then(async () => {
  ...
  private async deleteArticlePrices(articleId: number): Promise<boolean> {
    return new Promise<boolean>(async (resolve) => {
      this.db
        .execute(`DELETE FROM article_price WHERE article_id = ?;`, [articleId])
        .then(() => resolve(true))
        .catch(() => resolve(false));
    });
  }

  private async deleteArticleFeatureValues(
    articleId: number
  ): Promise<boolean> {
    return new Promise<boolean>(async (resolve) => {
      this.db
        .execute(`DELETE FROM article_feature WHERE article_id = ?;`, [
          articleId,
        ])
        .then(() => resolve(true))
        .catch(() => resolve(false));
  ...
  private async deleteArticleCartRecord(articleId: number): Promise<boolean> {
    return new Promise<boolean>(async (resolve) => {
      this.db
        .execute(`DELETE FROM cart_article WHERE article_id = ?;`, [articleId])
        .then(() => resolve(true))
        .catch(() => resolve(false));
    });
  }
