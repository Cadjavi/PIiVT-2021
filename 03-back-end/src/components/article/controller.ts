...
class ArticleController extends BaseController {
  ...
  private async uploadFiles(
    ...
    if (!req.files || Object.keys(req.files).length === 0) {
      res
        .status(400)
        .send(
          "You must upload at lease one and a maximum of " +
    ...
    for (const fileKey of fileKeys) {
      ...
      if (result.isOk === false) {
        res
          .status(400)
          .send(`Error with image ${fileKey}: "${result.message}".`);
        return [];
      }
      ...
      const imagePath =
        Config.fileUpload.uploadDestinationDirectory +
  ...
  public async deleteArticlePhoto(req: Request, res: Response) {
    ...
    const result = await this.services.articleService.deleteArticlePhoto(
      articleId,
      photoId
    );
  ...
  public async addArticlePhotos(req: Request, res: Response) {
    ...
    res.send(
      await this.services.articleService.addArticlePhotos(
        articleId,
        uploadedPhotos
      )
    );
