...
export default class CartService extends BaseService<CartModel> {
  protected async adaptModel(
    ...
    if (options.loadUser) {
      item.user = (await this.services.userService.getById(
        item.userId
      )) as UserModel;
    }
  ...
  public async getById(
    ...
    return (await super.getByIdFromTable<CartModelAdapterOptions>(
      "cart",
      cartId,
      options
    )) as CartModel | null;
  ...
  private async add(userId: number): Promise<CartModel | IErrorResponse> {
    return new Promise<CartModel | IErrorResponse>(async (resolve) => {
      this.db
        .execute(`INSERT cart SET user_id = ?;`, [userId])
        .then(async (res) => {
          const insertData = res[0] as any;
          const newCartId = +insertData?.insertId;
          resolve(
            await this.getById(newCartId, {
              loadUser: true,
            })
          );
        })
  ...
  public async getAllCartsByUserId(
    userId: number,
    options: Partial<CartModelAdapterOptions> = {}
  ): Promise<CartModel[]> {
    return (await super.getAllByFieldNameFromTable<CartModelAdapterOptions>(
      "cart",
      "user_id",
      userId,
      options
  ...
  public async getLatestCartByUserId(
    userId: number,
    options: Partial<CartModelAdapterOptions> = {}
  ): Promise<CartModel> {
    const [rows] = await this.db.execute(
  ...
  public async addArticleToLatestCartByUserId(
    ...
    const filteredArticles = cart.articles.filter(
  ...
  public async makeOrder(userId: number): Promise<CartModel | IErrorResponse> {
    return new Promise<CartModel | IErrorResponse>(async (resolve) => {
      ...
      this.db
        .execute(`INSERT INTO \`order\` SET cart_id = ?;`, [cart.cartId])
        .then(async () => {
          resolve(
            await this.getById(cart.cartId, {
              loadArticles: true,
              loadOrder: true,
              loadUser: true,
            })
  ...
  public async setOrderStatus(
    cartId: number,
    data: IOrderStatus
  ): Promise<CartModel | IErrorResponse> {
    return new Promise<CartModel | IErrorResponse>(async (resolve) => {
      ...
      this.db
        .execute(`UPDATE \`order\` SET \`status\` = ? WHERE order_id = ?;`, [
          data.status,
          cart.order.orderId,
        ])
        .then(async () => {
