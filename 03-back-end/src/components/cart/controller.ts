...
export default class CartController extends BaseController {
  private isCallerUser(req: Request, res: Response): boolean {
    if (req.authorized?.role !== "user") {
      res.status(403).send("This action is only available to user role. ");
      return false;
    }
  ...
  public async getCurrentUserCart(req: Request, res: Response) {
    ...
    res.send(
      await this.services.cartService.getLatestCartByUserId(req.authorized?.id)
    );
  }
  ...
  public async getAllOrdersForCurrentUser(req: Request, res: Response) {
    ...
    res.send(
      await this.services.cartService.getAllOrdersByUserId(req.authorized?.id)
    );
  }
