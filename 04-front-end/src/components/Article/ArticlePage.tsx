...
export default class ArticlePage extends BasePage<ArticlePageProperties> {
  ...
  private getArticleData() {
    ArticleService.getArticleById(this.getArticleId()).then((res) => {
      this.setState({
        data: res,
      });
    });
  ...
  renderMain(): JSX.Element {
    ...
    return (
      <>
        <h1>
          <Link to={"/category/" + article.categoryId}>&lt; Back</Link> |{" "}
          {article.title}
        </h1>
            ...
            <Card className="mb-3">
              <Row>
                {article.photos.map((photo) => (
                  <Col
                    key={"article-photo-" + photo.photoId}
                    xs={12}
                    sm={6}
                    md={4}
                    lg={3}
