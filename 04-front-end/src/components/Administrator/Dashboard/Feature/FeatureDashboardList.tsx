...
export default class FeatureDashboardList extends BasePage<FeatureDashboardListProperties> {
  ...
  private onChangeFeatureNameInput(
    featureId: number
  ...
  private getFeatureEditButtonClickHandler(featureId: number): () => void {
    return () => {
      FeatureService.editFeature(
        featureId,
        this.getCurrentFeatureName(featureId)
      ).then((res) => {
            ...
            {this.state.category.features
              ...
              .map((f) => (
                <tr key={"feature-row-" + f.featureId}>
                  ...
                  <td>{this.state.featureMessages.get(f.featureId)}</td>
                  <td>
                    <Button
                      variant="secondary"
                      size="sm"
                      onClick={this.getFeatureEditButtonClickHandler(
                        f.featureId
                      )}
                    >
                      Update
